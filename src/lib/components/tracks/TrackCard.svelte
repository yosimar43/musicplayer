<script lang="ts">
  ...
  let { track, index, visible, onclick }: { track: Track; index: number; visible: boolean; onclick?: () => void } = $props();
  ...
  $effect(() => {
    if (visible && cardRef) {
      ...
      gsap.to(cardRef, { opacity: 1, y: 0, duration: 0.3, delay: index % 10 * 0.05 });
    }
  });
